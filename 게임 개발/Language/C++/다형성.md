...
// - 오버라이딩 ( Overriding ) = 재정의 = 부모 클래스의 함수를 지식 클래스에서 재정의
...
// 그런데 실제 객체가 어떤 타입인지 어떻게 알고 가상함수를 호출해준걸까?
// - 가상 함수 테이블 ( vftable )
...
public:
	...
	// 순수 가상 함수는 0을 붙여준다.
	// 반드시 쓸거면 재정의를 해야한다는 강제사항을 주는 문법이다.
...
int main()
{
	// Player p; 순수 가상 함수를 사용하기에 추상클래스로 간주되어 따로 객체선언이 불가능하다. 
	// MovePlayer(&p); // 플레이어는 플레이어다? YES
	// MoveKnight(&p); // 플레이어는 기사다? NO

	// 최상위 객체가 Player 이기 때문에 이렇게도 가능하네
	Knight k;
